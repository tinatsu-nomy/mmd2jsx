...
遊太郎様作成の MMD2AE.EXE の実行結果から動作を推定し、AI によるコード生成を利用して開発しました。

また、開発時に AI が Inwerwm 様作成の MikuMikuMethods の C# ソースコードを参照しています。
...
本プロジェクトの独自のコードおよびドキュメントについて、著作者が許諾権限を持つ範囲を [BSD Zero Clause License（0BSD）](LICENSE) で提供します。
0BSD の対象部分は、商用・非商用を問わず使用・複製・改変・再配布でき、著作権表示やライセンス文の保持を条件としません。ソフトウェアは無保証です。

ただし、MikuMikuMethods やその同梱コードに由来するコピー・移植・翻案部分には、元の MIT ライセンスの条件が適用されます。第三者の著作権表示・許諾文を本プロジェクトの 0BSD に置き換えるものではありません。

参照元と本プロジェクトのコードの対応関係は未監査です。このため、現段階では第三者由来部分を含み得るパッケージとして保守的に扱い、Cargo のライセンス表記を `0BSD AND MIT` とし、MikuMikuMethods および同梱の MMDataIO の MIT 通知を保持する方針としています。これは AI による参照だけでコピー・翻案の存在を断定するものではありません。
詳細と配布時の扱いは [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) を参照してください。

本プロジェクト独自の出力用 JSX コードについても、著作者が許諾権限を持つ範囲を 0BSD で提供します。ただし、出力に第三者の保護されるコードが含まれる場合、その部分の条件は別途適用されます。入力するモデル・モーション等の権利や利用条件は、変換によって変更されません。
依存ライブラリには、それぞれのライセンスが適用されます。

上記の参考表記・謝辞は、0BSD の対象部分に追加のクレジット表示義務を設けるものではありません。
